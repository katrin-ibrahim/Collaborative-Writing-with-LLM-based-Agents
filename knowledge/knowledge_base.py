    ...
    def _retrieve_from_wiki(
        ...
        try:
            search_variations = [
                ...
                else topic,  # Remove last word
                topic.split()[0]
                if len(topic.split()) > 1
                else topic,  # Just first word
            ]
            ...
            if not search_results:
                print(
                    f"Warning: Could not find any Wikipedia pages related to '{topic}'"
                )
                return []
...
class KnowledgeBase:
    ...
    def to_outline(self) -> Dict[str, Any]:
        ...
        return {"title": self.topic, "sections": sections}
    ...
    def add_node(
        ...
        node = KnowledgeNode(
            content=content, source=source, confidence=confidence, metadata=metadata
        )
