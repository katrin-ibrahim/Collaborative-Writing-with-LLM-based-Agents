...
class KnowledgeBase:
    ...
    def create_category(self, category_name: str, category_description: str = "") -> KnowledgeNode:
    ...
    def assign_results_to_category(self, search_results: List[SearchResult], category_node: KnowledgeNode) -> None:
        """Assign specific search results to a category."""
        for result in search_results:
            category_node.add_search_result(result)

    def find_most_similar_results(self, target_text: str, search_results: List[SearchResult], top_k: int = 5) -> List[SearchResult]:
