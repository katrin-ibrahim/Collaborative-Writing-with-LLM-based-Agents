...
class SelfRAGRunner:
    ...
    def _check_self_rag_installation(self) -> bool:
        ...
        return os.path.exists(self.self_rag_path) and os.path.exists(
            os.path.join(self.self_rag_path, "retrieval_lm")
        )
        ...
        if not self._check_self_rag_installation():
            ...
            subprocess.run(
                [
                    "git",
                    "clone",
                    "https://github.com/AkariAsai/self-rag.git",
                    self.self_rag_path,
                ],
                check=True,
            )
...
Respond with only YES or NO."""
    ...
    def _retrieve_passages(
        self, topic: str, num_passages: int = 5
    ) -> List[Dict[str, str]]:
...
Article:"""
    ...
    def _self_reflect(
        self, topic: str, content: str, passages: List[Dict[str, str]]
    ) -> Dict[str, Any]:
