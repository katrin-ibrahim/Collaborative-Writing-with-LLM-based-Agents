...
class SearchToolkit:
    ...
    def search_all_sources(
        self, query: str, wiki_results: int = 3, web_results: int = 3
    ) -> List[SearchResult]:
        """Search all available sources."""
        return self.search_engine.search_all_sources(query, wiki_results, web_results)

    def verify_claim(
        self, claim: str, context_results: List[SearchResult] = None
    ) -> Dict[str, Any]:
        """
        ...
        search_results = self.search_all_sources(
            verification_query, wiki_results=2, web_results=3
        )
