...
class ContentFilter:
    ...
    def deduplicate_results(
        ...
        This function assumes the input is a list of dictionary results,
        which matches the data structure used by the BaseRetriever.

        Args:
            results: List of search result dictionaries to deduplicate

        Returns:
            Deduplicated search result dictionaries
        """
        if not results:
            return results
    ...
    def _deduplicate_dict_results(
        ...
        for result in results:
            ...
            if content:
                ...
                if content_str:
                    # Note: Using built-in hash() is safe for string uniqueness check
    ...
    # Removed _deduplicate_string_results as it is not needed by the BaseRetriever's usage pattern.
