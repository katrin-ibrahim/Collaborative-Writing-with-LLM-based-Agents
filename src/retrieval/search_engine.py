...
class SearchEngine:
    ...
    def search_all_sources(
        ...
        logger.info(
            f"Retrieved {len(unique_results)} search results for query: {query}"
        )
        ...
        try:
            ...
            for variation in entity_variations:
                snippets = self.wiki_retriever.get_wiki_content(
                    variation,
                    max_articles=max(1, max_results // len(entity_variations)),
                    max_sections=2,
                )
            ...
            for snippet in all_snippets:
                ...
                search_results.append(
                    SearchResult(
                        content=content, source=source, relevance_score=relevance
                    )
                )
