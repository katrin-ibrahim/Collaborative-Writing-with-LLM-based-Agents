...
class WriterAgent:
    ...
    def __init__(self, config: WriterConfig, args: RunnerArgument):
        ...
        self.api_url = (
            f"https://router.huggingface.co/novita/v3/openai/chat/completions"
        )
        self.headers = (
            {"Authorization": f"Bearer {config.hf_token}"} if config.hf_token else {}
        )
        logging.info(
            f"Using HF Inference API with token: {config.hf_token is not None}"
        )
    ...
    def research(self) -> List[Dict[str, Any]]:
        ...
        retrieve_count = (
            self.args.retrieve_top_k if hasattr(self.args, "retrieve_top_k") else 3
        )
        wiki_snippets = self.wiki_retriever.get_wiki_content(
            self.topic, max_articles=retrieve_count
        )
    ...
    def organize_knowledge(self) -> None:
        ...
        snippets_text = "\n\n".join(
            [f"Snippet {i+1}: {s['content']}" for i, s in enumerate(all_snippets)]
        )
        print(
            f"Snippets text for LLM:\n{snippets_text[:500]}..."
        ...
        try:
            ...
            print(
                "Failed to parse knowledge organization response. Keeping flat structure."
            )
    ...
    def draft_section(self, section: Dict[str, Any]) -> str:
        ...
        relevant_snippets = []
        if (
            not hasattr(self.knowledge_base, "root")
            or not self.knowledge_base.root.children
        ):
        ...
        for node in self.knowledge_base.root.children:
            if (
                title.lower() in node.title.lower()
                or node.title.lower() in title.lower()
            ):
                relevant_snippets.extend(node.snippets)

            for child in node.children:
                if (
                    title.lower() in child.title.lower()
                    or child.title.lower() in title.lower()
                ):
